...
class Apartment:
    ...
    def check_eligibility(self, some_name, proof_of_identity, proof_of_income):
        ...
        if proof_of_identity == "yes" and proof_of_income == "yes":
            print(f"{some_name}, you are eligible to lease an apartment in "
            "College Park.")
            self.proof_of_identity_boolean = True
            self.proof_of_income_boolean = True

        elif proof_of_identity == "yes" and proof_of_income == "no":
            print(f"{some_name}, you are not eligible to lease an apartment "
            "in College Park." 
            ...
            return(f"{some_name}, you are not eligible to lease an apartment "
            "in College Park.")
        
        result = "Identity and income verified." if (proof_of_identity == True 
        and proof_of_income == True) else "Identity and/or income not verified"
        print(result)
    ...
    def find_shared_group_apartment(self, num_people, some_apartment, budget):
        """
        Finds apartments that can accommodate a group of people with the given 
        criteria.

        Args:
        - num_people (int): The number of people who want to live together 
        in the apartment.
        - some_apartment (str): The apartment chosen by the user.
        - budget (float): The monthly rent per person in the group for 
    ...
    # Assume we have a list of available apartments with their details
        chosen_apartment_df =  self.apartments_df[
            self.apartments_df['Apartment Name'] == self.chosen_apartment]
        #print(chosen_apartment_df)
        chosen_rooms_available_df = chosen_apartment_df[
            chosen_apartment_df['Number of Rooms Available']>= self.num_people ]
        print("\nThese are the rooms available that can fit the number of tenants:")
        print(chosen_rooms_available_df)

        chosen_apartment_number = int(input("\nWhich of the apartment units "
        "listed best fit your needs?"
        "\nPlease specify the Apartment Number of the unit shown under "
        "the Apartment Number column (e.g. 500):" ))
        apartment_number_df = chosen_apartment_df[
            chosen_apartment_df['Apartment Number']== chosen_apartment_number]
        ...
        #f-strings containing expressions
        print("This is the monthly rent that each of the tenants "
        f"have to pay (including you): ${budget}.")
